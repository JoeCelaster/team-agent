...
export function SimLauncher() {
  ...
  return (
    <div className="fixed bottom-6 left-6 z-50 flex flex-col items-start gap-3 font-sans">
      {toast && (
        <div className="animate-toast-in w-72 rounded-xl border border-indigo-500/40 bg-surface p-3.5 shadow-2xl shadow-indigo-950/40">
          <div className="flex items-start gap-3">
            <div className="rounded-lg bg-indigo-500/10 p-2 text-indigo-400">
              <Mail className="h-4 w-4" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-foreground">Invite email sent</p>
              <p className="mt-0.5 truncate text-xs text-muted">To: {toast.to}</p>
              <button
                onClick={() => {
            ...
            <button
              ...
              className="-m-1 rounded-md p-1 text-faint transition-colors hover:text-muted"
              aria-label="Dismiss"
            >
      ...
      <Link
        href="/sim"
        className="group inline-flex items-center gap-2 rounded-full border border-border bg-surface/90 py-2 pl-3 pr-3.5 text-xs font-medium text-muted shadow-lg shadow-black/30 backdrop-blur-sm transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-indigo-500/40 hover:text-foreground"
      >
        <span className="relative inline-flex">
          <FlaskConical className="h-3.5 w-3.5 text-indigo-400" />
          {unread && (
            <span className="absolute -right-1 -top-1 h-1.5 w-1.5 rounded-full bg-indigo-400 ring-2 ring-surface" />
          )}
        </span>
        Demo controls
      </Link>
