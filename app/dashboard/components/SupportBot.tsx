...
export function SupportBot({ session, onAccessRequested }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--surface)] border border-zinc-800 rounded-2xl overflow-hidden">
      {/* Header */}
      <div className="flex items-center gap-2.5 px-4 py-3.5 border-b border-zinc-800 shrink-0">
        <div className="w-7 h-7 rounded-lg bg-indigo-600 flex items-center justify-center">
          <Bot className="w-4 h-4 text-white" />
        </div>
        <div>
          <p className="text-xs font-bold text-zinc-200">Onboarding Assistant</p>
          <p className="text-[10px] text-zinc-500">Multi-provider AI · can take actions</p>
        </div>
      ...
      <form
        ...
        className="shrink-0 flex items-center gap-2 px-3 py-3 border-t border-zinc-800"
      >
        <input
          value={input}
          ...
          disabled={isLoading}
          className="flex-1 bg-zinc-800 border border-zinc-700 focus:border-indigo-500 text-zinc-200 text-xs rounded-xl px-3.5 py-2.5 outline-none transition-colors placeholder:text-zinc-600 disabled:opacity-50"
        />
        <button
          type="submit"
