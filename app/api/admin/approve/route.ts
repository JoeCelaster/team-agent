...
export async function POST(request: Request) {
  ...
  // Get the access_request row to know employee_id + resource_id
  ...
  await supabaseServer
    .from("access_requests")
    .update({ status: "approved", admin_note, reviewed_at: new Date().toISOString(), reviewed_by_id: reviewer_id })
    .eq("id", request_id);
